...
class ConnectDB:
    def __init__(self,user,pwd,database,host= 'localhost',port = 5432):
        self.engine = psycopg2.connect(f'postgresql://{user}:{pwd}@{host}:{port}/{database}')    
        try:
            self.connection = self.engine.cursor()
            #Logger.emit(f'Successfully opened connection on {host}', 'INFO')
        except Exception as ex:
            #Logger.emit(f'Error stablishing connection on {host}: {ex}', 'ERROR')
